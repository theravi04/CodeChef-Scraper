...
function App() {
  ...
  const sendRequest = () => {
    ...
    axios
      .post("http://localhost:4000/", { username })
      .then((response) => {
        ...
        {userData && (
          <div className="w-full md:w-3/4 lg:w-2/3 xl:w-1/2 mt-8 p-4 rounded-lg text-white text-left flex flex-col md:flex-row">
            {userData.profileImgUrl && (
              <div className="md:ml-4 p-4 flex items-center justify-center" style={{ flexBasis: "35%" }}>
                <img
            ...
            {/* Left side: User Data */}
            <div className="flex-grow p-4 mb-4 md:mb-0 rounded-lg" style={{ flexBasis: "65%" }}>
              <h2 className="font-bold text-2xl mb-4">CodeChef User</h2>
              <p><strong>User ID:</strong> {userData.userId || "N/A"}</p>
              <p><strong>User Rating:</strong> {userData.userRating || "N/A"}</p>
              <p><strong>User Stars:</strong> {userData.userStar || "N/A"}</p>
              <p><strong>Total Contest Given:</strong> {userData.contestCount || 0}</p>
              <p><strong>Country:</strong> {userData.additionalInfo.find(info => info.labelText === "Country:")?.spanText || "N/A"}</p>
              <p><strong>Student/Professional:</strong> {userData.additionalInfo.find(info => info.labelText === "Student/Professional:")?.spanText || "N/A"}</p>
              <p><strong>Institution:</strong> {userData.additionalInfo.find(info => info.labelText === "Institution:")?.spanText || "N/A"}</p>
            </div>
